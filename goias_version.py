...
def get_estado_from_municipio(municipio):
    """Extrai o estado do formato 'Estado,Município'"""
    if pd.isna(municipio):
        return None
    municipio_str = str(municipio).strip()
    if "," in municipio_str:
        estado = municipio_str.split(",")[0].strip()
        return estado
    return None
...
def load_data(csv_file):
    ...
    # ✅ Adiciona coluna de estado extraindo de 'Estado,Município'
    df['estado'] = df['municipio'].apply(get_estado_from_municipio)
...
# ✅ FILTRO AUTOMÁTICO - APENAS GOIÁS
if not df.empty:
    df_filtered = df[df['estado'] == 'Goias'].copy().reset_index(drop=True)
else:
    df_filtered = df.copy()
