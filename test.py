            ...
            if fotos_validas:
                ...
                if len(fotos_validas) > 1:
                    ...
                    for tab, (foto_id, foto_path) in zip(tabs, fotos_validas):
                        with tab:
                            st.image(foto_path, caption=f"Foto {foto_id}", use_container_width=True)
                else:
                    # Se tem só 1 foto, mostra direto
                    st.image(fotos_validas[0][1], caption=f"Foto da Amostra {int(selected_row['ID'])}", use_container_width=True)
            else:
                st.markdown("""
